...
export async function backup() {
  const filename = `verger.${dayjs().format('YYYYMMDDHHmm')}.zip`
...
export async function backupToWebdav({
  ...
  const backupFileName = customFileName || `verger.${timestamp}.${hostname}.${deviceType}.zip`
...
export async function handleData(data: Record<string, any>) {
  if (data.version === 1) {
    ...
    await localStorage.setItem(
      'persist:verger',
      data.localStorage['persist:cherry-studio'] || data.localStorage['persist:verger']
    )
